...
export async function onRequest(context) {
  ...
  if (method === 'POST') {
    try {
      ...
      const { id, person_name, violation_type, points, location, item_name, product_number, serial_number, photo_base64, created_at } = body;
      ...
      await env.DB.prepare(
        `INSERT INTO tickets (id, person_name, violation_type, points, location, item_name, product_number, serial_number, photo_base64, status, appeal_flagged, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'open', 0, ?)`
      ).bind(id, person_name, violation_type, points, location, item_name || null, product_number || null, serial_number || null, photo_base64, created_at).run();
