...
export async function onRequest(context) {
  ...
  try {
    ...
    if (ticket.appeal_flagged) return new Response(JSON.stringify({ error: 'Appeal already submitted.' }), { status: 400, headers });

    await env.DB.prepare(`UPDATE tickets SET appeal_note = ?, appeal_flagged = 1 WHERE id = ?`).bind(note, id).run();
